...
When a Production change crosses repository boundaries, use each repository for the facts it owns. Identify all governing authorities needed for the specific mutation. Do not let one repository's documentation silently replace another repository's ownership.
